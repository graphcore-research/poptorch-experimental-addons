...
class ReplicatedAllReduceTPOp : public ReplicatedAllReduceOp {
   public:
    ...
    std::vector<std::unique_ptr<Op>> getGradOps() override {
        ...
        result.push_back(
            std::make_unique<ReplicatedAllReduceTPOp>(opid, op, grouping, !backwards, settings));
        return result;
    }
...
static OpDefinition ReplicatedAllReduceTPOpDef(
    {OpDefinition::Inputs({{"X", T}}), OpDefinition::Outputs({{"Y", T}}),
...
static OpCreator<ReplicatedAllReduceTPOp> ReplicatedAllReduceTPOpCreator(
    ...
    [](const OpCreatorInfo& info) {
        CollectiveOperator op =
            static_cast<CollectiveOperator>(info.attributes.getAttribute<Attributes::Int>(
        ...
        return std::make_unique<ReplicatedAllReduceTPOp>(
            info.opid, op, grouping, info.attributes.getAttribute<Attributes::Int>("backwards"),
            info.settings);
    },
    true);
