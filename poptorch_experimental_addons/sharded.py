...
def rowcolrow_sharded_matmul(
    X: torch.Tensor, Y: torch.Tensor, rg_info: ReplicaGroupingInfo, num_chunks: int = 1
) -> Any:
    """
    ...
    result = torch.zeros((X_local_outer_dim, Y_local_outer_dim * rg_info.group_size))
    ...
    for i in range(num_chunks):
        ...
        Yg = all_gather_cross_replica(
            torch.index_select(Y, dim=1, index=index).squeeze(), rg_info
        )
...
def repcolcol_sharded_matmul(
    X: torch.Tensor, Y: torch.Tensor, rg_info: ReplicaGroupingInfo
) -> Any:
    """
...
def colrowrep_sharded_matmul(
    ...
    out = X @ Y
    return all_reduce_cross_replica_sum(out, rg_info)
